...
const CreateScreen = () => {  
  ...
  const handleSubmit = async () => {
    try {
      await axios.put(`http://${config.ipApi}:3000/api/map/${id}`, { nombre: Nombre, latitud: origin.latitude, longitud: origin.longitude });
      Alert.alert('Success', 'Item created successfully');
      router.push('Sargento/mapas'); // Navega de vuelta a la pantalla de inicio
    } catch (error) {
  ...
  if (isLoading) {
    ...
    <View style={styles.todo}>
      <View style={styles.container}>
        <Pressable style={styles.regresar} onPress={() => router.push('Sargento/mapas')}>
    ...
    ) // Muestra la pantalla de carga mientras los datos se están cargando
  }
    ...
    <View style={styles.todo}>
      <View style={styles.container}>
        ...
        <View style={styles.mapa}>
          <Text style={styles.titulo}>Edita Incurcio</Text>
          <MapView 
          style={styles.map}
          initialRegion={{
          ...
          <Text style={styles.text}>Nombre de la ubicacion</Text>
